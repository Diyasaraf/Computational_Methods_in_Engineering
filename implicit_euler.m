...
for i=1:n
 A(i+1)=A(i)+h*f1(A(i+1),B(i+1),C(i+1),D(i+1));
 B(i+1)=B(i)+h*f2(A(i+1),B(i+1),C(i+1),D(i+1));
 C(i+1)=C(i)+h*f3(A(i+1),B(i+1),C(i+1),D(i+1));
 D(i+1)=D(i)+h*f4(A(i+1),B(i+1),C(i+1),D(i+1));
 t(i+1)=t(i)+h;
end
figure(3)
plot(t,A);
